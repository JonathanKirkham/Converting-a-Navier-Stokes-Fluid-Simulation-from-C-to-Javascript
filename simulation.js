...
const poisson = (p, rhs, flag, imax, jmax,
    delx, dely, eps, itermax, omega,
    res, ifull) => {
    let i, j, iter;
    let add, beta_2, beta_mod;
    let p0 = 0.0;

    let rb; /* Red-black value. */

    let rdx2 = 1.0 / (delx * delx);
    let rdy2 = 1.0 / (dely * dely);
            ...
            for (i = 1; i <= imax; i++) {
                for (j = 1; j <= jmax; j++) {
                    ...
                    if (flag[i][j] == (C_F | B_NSEW)) {
                        ...
                        let eps_E = (flag[i + 1][j] & C_F) ? 1 : 0;
                        let eps_W = (flag[i - 1][j] & C_F) ? 1 : 0;
                        let eps_N = (flag[i][j + 1] & C_F) ? 1 : 0;
                        let eps_S = (flag[i][j - 1] & C_F) ? 1 : 0;
