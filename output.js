...
const setPixleRect = (data, x, y, rowlength, r, g, b) => {
    let i, j;
    for (i = 0; i < pixelSize; i++) {
        for (j = 0; j < pixelSize; j++) {
            let pixelIdx = (y * pixelSize + j) * rowlength + (x * pixelSize + i)
            data[4 * pixelIdx + 0] = r;
            data[4 * pixelIdx + 1] = g;
            data[4 * pixelIdx + 2] = b;
...
const write_ppm = (u, v, p, flag,
    ...
    let delx = xlength / imax;
    let dely = ylength / jmax;
    ...
    let imageData = m_context.getImageData(0, 0, imax * pixelSize, jmax * pixelSize);
    for (j = 1; j < jmax + 1; j++) {
        for (i = 1; i < imax + 1; i++) {
            var r, g, b;
            if ((flag[i][j] & C_F)) {
